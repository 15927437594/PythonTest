...
def login():
    try:
        ...
        response = requests.post(url=url, data=json.dumps({'name': 'zbs', 'password': '123456'}), headers=headers)
...
def get_wip_statics(token, station_no):
    url = 'http://10.8.1.20/' + 'sfcapi/1.0/wip-statics/'
    print('iqc_test_report url=%s' % url)
    try:
        print('token=', token)
        print('url=', url+station_no)
        headers = {"Authorization": "Bearer %s" % token}
        response = requests.get(url=url+station_no, headers=headers, timeout=3)
        status_code = response.status_code
        response_json = response.json()
        print('iqc_test_report status_code=%s, response_json=%s' % (status_code, response_json))
        return response_json
...
get_wip_statics(token, "E360CE08922611ED9880000C29AAA101")
